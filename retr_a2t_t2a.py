...
class AudioCaptionDataset(torch.utils.data.Dataset):
    def __init__(self, cfg, split, random_crop=True):
        super().__init__()
        self.cfg = cfg
        self.split = split
...
def to_embeddings(ar, data_loader, device, _id=None, reliable_count=10):
    ...
    for i in tqdm(range(len(dataset))):
        ...
        with torch.no_grad():
            ...
            cur_emb = ar.encode_audio(X.unsqueeze(0))
    ...
    return audio_embs, cap_embs


def create_retrieval_data(cfg):
    #train_dataset = AudioCaptionDataset(cfg, 'train', random_crop=True)
    valid_dataset = AudioCaptionDataset(cfg, 'val', random_crop=False)
    test_dataset = AudioCaptionDataset(cfg, 'test', random_crop=False)
...
def audio_text_retrieval(config_file, task, options=''):
    ...
    df = pd.DataFrame({'model': [cfg.id], 'task': [task],
                       'a2t_r1': results_a2t[:1], 'a2t_r5': results_a2t[1:2], 'a2t_r10': results_a2t[2:3],
                       't2a_r1': results_t2a[:1], 't2a_r5': results_t2a[1:2], 't2a_r10': results_t2a[2:3]})
